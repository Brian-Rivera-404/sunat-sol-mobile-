  ...
  return (
    <ScrollView className="flex-1 bg-gray-50 dark:bg-gray-900">
      ...
      <View className="px-4 pt-6">
        {unreadCount > 0 && (
          <View className="bg-blue-50 dark:bg-blue-900 border border-blue-200 dark:border-blue-700 rounded-xl px-4 py-3 mb-4" accessibilityRole="alert" accessibilityLiveRegion="polite">
        ...
        {sorted.length === 0 ? (
          <View className="items-center justify-center py-16">
            ...
            <Text className="text-gray-500 dark:text-gray-400">{t('inbox_empty')}</Text>
          </View>
        ) : (
          sorted.map((msg) => (
            <TouchableOpacity
              key={msg.id}
              className={`bg-white dark:bg-gray-800 rounded-xl p-4 mb-3 shadow-sm ${!msg.leido ? 'border-l-4 border-[#002f5d]' : ''}`}
              onPress={() => {
              ...
              <View className="flex-row justify-between items-start mb-1">
                <View className="flex-1 mr-2">
                  <View className="flex-row items-center">
                    {!msg.leido && <View className="w-2 h-2 rounded-full bg-[#002f5d] mr-2" />}
                    <Text className={`text-sm font-semibold ${!msg.leido ? 'text-gray-900 dark:text-gray-100' : 'text-gray-600 dark:text-gray-400'}`}>{msg.titulo}</Text>
                  </View>
                  <Text className={`text-xs mt-1 ${!msg.leido ? 'text-gray-600 dark:text-gray-400' : 'text-gray-400 dark:text-gray-500'}`}>{msg.cuerpo}</Text>
                </View>
                <Text className="text-xs text-gray-400 dark:text-gray-500">{formatearFecha(msg.fecha)}</Text>
              </View>
              {!msg.leido && (
                <View className="mt-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                  <Text className="text-xs text-[#002f5d] dark:text-blue-400 font-semibold">{t('inbox_ask_assistant')} {'\u203A'}</Text>
                </View>
              )}
            </TouchableOpacity>
          ))
