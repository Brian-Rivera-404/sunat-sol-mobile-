  ...
  return (
    <ScrollView className="flex-1 bg-gray-50 dark:bg-gray-900">
      ...
      <View className="px-4 pt-6">
        <View className="bg-white dark:bg-gray-800 rounded-xl p-5 shadow-sm mb-4" accessibilityLabel={`${t('calendar_ruc_digit')}: ${lastDigit}`}>
          <View className="flex-row items-center mb-3">
            <Text className="text-2xl mr-2">{'\uD83D\uDCC5'}</Text>
            <View>
              <Text className="text-sm text-gray-500 dark:text-gray-400">{t('calendar_ruc_digit')}</Text>
              <Text className="text-lg font-bold text-gray-800 dark:text-gray-100">{lastDigit}</Text>
            </View>
          </View>
          <Text className="text-xs text-gray-500 dark:text-gray-400 leading-4">{t('calendar_ruc_explain')}</Text>
        </View>
        ...
        <View className="mb-4">
          ...
          {DEADLINES.map((dl, i) => {
            ...
            return (
              <View
                key={i}
                className={`bg-white dark:bg-gray-800 rounded-xl p-4 mb-2 flex-row items-center shadow-sm ${isCurrent ? 'border-2 border-[#002f5d] dark:border-blue-400' : ''} ${isPast ? 'opacity-50' : ''}`}
                accessibilityLabel={`${dl.mes} ${dl.dia}: ${t(dl.label)}`}
              >
                <View className={`w-10 h-10 rounded-full ${isCurrent ? 'bg-[#002f5d]' : isPast ? 'bg-gray-200 dark:bg-gray-700' : 'bg-blue-100 dark:bg-blue-900'} items-center justify-center mr-3`}>
                  <Text className={`text-sm font-bold ${isCurrent ? 'text-white' : isPast ? 'text-gray-400' : 'text-[#002f5d] dark:text-blue-300'}`}>{dl.dia}</Text>
                </View>
                <View className="flex-1">
                  ...
                  <Text className={`text-xs ${isPast ? 'text-gray-400 dark:text-gray-500' : 'text-gray-500 dark:text-gray-400'}`}>{t(dl.label)}</Text>
                </View>
                {isCurrent && <Text className="text-[#002f5d] dark:text-blue-400 text-xs font-semibold">{t('calendar_current')}</Text>}
              </View>
            )
          })}
        </View>
        ...
        <TouchableOpacity
          className="bg-white dark:bg-gray-800 rounded-xl p-4 flex-row items-center justify-between mb-10 shadow-sm"
        ...
        >
          <View className="flex-row items-center">
            ...
            <View>
              ...
              <Text className="text-gray-500 dark:text-gray-400 text-xs">{t('calendar_reminder_desc')}</Text>
            </View>
          </View>
